...
class Context{
  ...
  const static char* contextFilename;
  const static char* diffFilename;
 ...
 public:
  ...
  void writeDiff(Plottables p) {
    ofstream f(diffFilename, ios::out);
    f<<"~POINTS"<<endl;
  ...
  void writeContext() {
    ofstream f(contextFilename, ios::out);
    f<<"~POINTS"<<endl;
...
const char* Context::contextFilename = "./context.txt";
